...
「Full Stack FastAPI テンプレート」が FastAPI により用意されている．
...
-   `SECRET_KEY`: `python -c "import secrets; print(secrets.token_urlsafe(32))"` で得られる文字列を設定する
-   `FIRST_SUPERUSER`: 自分のメールアドレスを設定する
-   `FIRST_SUPERUSER_PASSWORD`: 8文字以上で設定する
-   `POSTGRES_PASSWORD`: なにがしか設定する
-   `BACKEND_CORS_ORIGINS`: サーバとして公開する自分IPアドレスが 192.168.11.13 であれば `http://192.168.11.13:5173/`を追加しておく(CORS)．

以下により Docker のコンテナ群を起動すれば，利用できる．

```bash
% docker compose up
```

上記IPアドレスであれば `http://192.168.11.13:5173/` にアクセスし，あとは予め設定した `FIRST_SUPERUSER` および `FIRST_SUPERUSER_PASSWORD` により管理者(Admin)としてログインできる．

なお，公式の [`README.md`](https://github.com/fastapi/full-stack-fastapi-template/blob/master/README.md) では，`git clone` だけでなく `copier` を用いて新規プロジェクトを生成する方法も紹介されている．`copier` を用いると，プロジェクト名などを対話的に設定し，`.env` もその内容に応じて更新される．`--trust` は，`.env` を更新するための生成後スクリプト(post-creation script)を実行するために必要である．
